...
if len(sys.argv)!=2:
	'''
	e.g. python unitTestAdversary.py 00m
	'''
	print "Usage: python unitTestAdversary_owner_10.py attackSample"
	exit()

ATTACK = sys.argv[1]

ROOT = '/home/tiantian/onecycleattack/Gait-Authentication-Attack/dtw_and_manhattan'
RESULTS_DIR = 'pcc_data_splitted/mimicry_ren_data_splitted'
TRAIN_DIR = 'pcc_data_splitted/owner_magnitude_ren_data_splitted'
...
def get_dataset(attackSample):
	...
	#attack_list = np.loadtxt(os.path.join(ROOT, 'attack_10.txt'))
	# srore the person we need to attack
	test_list = []
	with open (os.path.join(ROOT, 'attack_10.txt'), 'r') as f:
	...
	cycle_test_list = ['00','01','10','11','20','21','30','31','40','41']
	test_value_list = []
...
class myAuth(Authentication):
	...
	def oneSampleTest(self, attackSample):
		...
		if auth_result:
			with open(os.path.join(LOG_DIR, 'log_mimicry'), 'a') as f:
	...
	def authFunc(self, attackSample):
		...
		for cycle_train_name in cycle_train_list:
			train_values = []
			train_filepath = os.path.join(ROOT, TRAIN_DIR, self.victim, cycle_train_name)
