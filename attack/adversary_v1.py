...
class Adversary(object):
	...
	def run(self):
		...
		nearestDist = [-1 for i in range(dataNum)]
		...
		while tryNum <= limitedNum:
			...
			distance = []
			for i in range(dataNum):
				nearestDist[i] = self.calNearestDist(featureList[i], attackSample[-1], nearestDist[i])
				distance.append(nearestDist[i])
	...
	def calNearestDist(self, data, sample, preMin):
		'''
		@data: A single sample vector.
		@sample: sample which have been used as attack-samples before.
		@preMin: preversed minimum distance last turn.
