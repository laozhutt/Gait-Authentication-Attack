...
if len(sys.argv)!=2:
	'''
	e.g. python threshold_320.py ID130007
	'''
	print "Usage: python threshold_320.py attackSample"
	exit()
...
def get_dataset(attackSample):
	...
	attack_list = np.loadtxt(os.path.join(ROOT, 'attack.txt'))

	test_list = ['ID' + "%06d" % i for i in attack_list]
...
class myAuth(Authentication):
	...
	def __init__(self, DataSet, Victim, Num, Threshold):
	...
	def oneSampleTest(self, attackSample):
		...
		if auth_result:
			with open(os.path.join(TH_DIR, str(self.threshold)), 'a') as f:
...
if __name__ == '__main__':
	...
	th = 0.0
	while(th < 1):
		auth = myAuth(dataSet, attackSample , 100, th)
		ad = Adversary(auth)
		tryNum, pos = ad.run()
		th = th + 0.01
